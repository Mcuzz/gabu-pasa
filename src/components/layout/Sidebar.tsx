    ...
    <>
      ...
      >
        ...
        <div className="hidden h-full lg:flex lg:flex-col">
          ...
          {/* User Menu — solo visible cuando está expandido */}
          {expanded && (
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div
            className="relative z-10 h-full w-[88%] max-w-sm px-4 py-4 flex flex-col"
            style={{
              background:
...
function SidebarLink({
  ...
  return (
    <Link
      ...
      `}
    >
      <div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${isActive ? "bg-white/12" : "bg-white/10"}`}>
        <Image src={item.icon} alt={item.label} width={22} height={22} className="brightness-0 invert" />
      </div>
      {expanded && (
        <div className="min-w-0">
          <span className="block whitespace-nowrap text-sm font-semibold">{item.label}</span>
          <span className="block text-xs text-white/70">{isActive ? "Vista actual" : "Abrir seccion"}</span>
        </div>
      )}
    </Link>
  );
